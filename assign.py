...
# consts
VALID_SESSIONS = [1, 2, 3, 4, 5, 6, 7, 8, 9]
...
# mutables
responses = []
students = {} # dictionary of email to [ filled: bool, assigned : bool, grade : string, first_name : string, last_name : string, assignment : int ]
assignments = [0 for i in range(len(VALID_SESSIONS))]
print(session_caps)
...
# responses parsing
# format: timestamp, email, session 1, session 2, session 3
# session format: int (corresponds to VALID_SESSIONS)
...
with open('students.csv') as f_students:
	...
	for row in r_students:
		...
		students[row[0]] = [False, False] + row[1:]	# first pair: filled, assigned
...
def randomly_assign(student):
	...
	while True:
		lowest = -1
		lowest_val = 10000
		for cap_index in range(len(assignments)):
			if assignments[cap_index] < lowest_val and not cap_index in tried:
				lowest = cap_index
				lowest_val = assignments[cap_index]
		if assignments[lowest] < session_caps[lowest]:
...
for r_index in range(len(responses)):
	...
	for desire in response[1:]:
		if not desire in VALID_SESSIONS:
			continue
		if assignments[VALID_SESSIONS.index(desire)] < session_caps[VALID_SESSIONS.index(desire)]:
...
for location in session_locations:
	with open('rooms/' + location + '.csv', 'w') as f:
		writer = csv.writer(f)
		writer.writerow(['Email', 'Form Completed', 'Satisfied', 'Grade', 'Advisor', 'First Name', 'Last Name', 'Session Name', 'Location'])
		for student in students.keys():
			if session_locations[students[student][-1]] == location:
				writer.writerow([student] + students[student][:-1] + [session_names[students[student][-1]], session_locations[students[student][-1]]])
